...
class Modelo():
    ...
    def convolucion_imagenes(self):
        ...
        with tf.Session() as sess:
            ...
            # codigo para guardar el modelo SAVEDMODEL
            if self.save_model_configuration == True:
                if not os.path.exists(dir):
                    os.makedirs(dir)
                    print("-------CARPETA MODEL CREADA---------")

                ###########
                saver.save(sess, './' + dir + '/model{}.ckpt'.format(version))
                print("Exporting trained model to", dir)
    ...
    def placeholders(self, *args, **kwargs):
        ...
        """
        # Placeholders
        x = tf.placeholder(tf.float32, shape=[None, self.input_rows_numbers * self.input_columns_numbers], name='x_real')  # All images will be 60x60
        y_ = tf.placeholder(tf.float32, shape=[None, self.number_of_classes], name="y_")  # Number of labels
        keep_probably = tf.placeholder(tf.float32, name='keep_probably')  # Value of dropout. With this you can set a value for each data set
    ...
    def network_structure(self, input, *args, **kwargs):
        ...
        y_prediction = tf.identity(y_convolution, name='y_prediction')
        return y_prediction
    ...
    def model_evaluation(self, y_labels, y_prediction, *args, **kwargs):
        ...
        # Sure is axis = 1
        correct_prediction = tf.equal(tf.argmax(y_prediction, axis=1),tf.argmax(y_labels, axis=1), name='correct_prediction')  # Get Number of right values in tensor
    ...
    def train_model(self, *args, **kwargs):
        ...
        for epoch in range(self.num_epochs_count, self.epoch_numbers):  # Start with load value or 0
            ...
            for num_train in range(num_train_start, self.trains):  # Start with load value or 0
                ...
                feed_dict_train_dropout = {x: self.input_batch, y_labels: self.label_batch, keep_probably: self.train_dropout}
                # Setting values
